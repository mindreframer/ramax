defmodule PState.E2EMigrationTest do
  ...
  describe "RMX004_9A_T6: Per-track migration" do
    test "migrates 1500 cards efficiently" do
      ...
      {:ok, stored1} =
        pstate.adapter.get(pstate.adapter_state, pstate.space_id, "base_card:card1")

      {:ok, stored500} =
        pstate.adapter.get(pstate.adapter_state, pstate.space_id, "base_card:card500")

      {:ok, stored1500} =
        pstate.adapter.get(pstate.adapter_state, pstate.space_id, "base_card:card1500")
