defmodule EventStoreTest do
  ...
  describe "RMX005_4A: EventStore Public API" do
    ...
    test "RMX005_4A_T6: get_events/3 works with both ETS and SQLite adapters" do
      ...
      {:ok, _id1, _seq1, ets_store} =
        EventStore.append(ets_store, 1, "entity:1", "event.1", %{value: "a"})

      {:ok, _id2, _seq2, ets_store} =
        EventStore.append(ets_store, 1, "entity:1", "event.2", %{value: "b"})

      {:ok, _id3, _seq3, ets_store} =
        EventStore.append(ets_store, 1, "entity:2", "event.3", %{value: "c"})
      ...
      {:ok, _id1, _seq1, sqlite_store} =
        EventStore.append(sqlite_store, 1, "entity:1", "event.1", %{value: "a"})

      {:ok, _id2, _seq2, sqlite_store} =
        EventStore.append(sqlite_store, 1, "entity:1", "event.2", %{value: "b"})

      {:ok, _id3, _seq3, sqlite_store} =
        EventStore.append(sqlite_store, 1, "entity:2", "event.3", %{value: "c"})
    ...
    test "RMX005_4A_EXTRA_T2: get_all_events/2 retrieves all events" do
      ...
      {:ok, _id1, _seq1, store} =
        EventStore.append(store, 1, "entity:1", "event.1", %{value: "a"})

      {:ok, _id2, _seq2, store} =
        EventStore.append(store, 1, "entity:2", "event.2", %{value: "b"})

      {:ok, _id3, _seq3, store} =
        EventStore.append(store, 1, "entity:1", "event.3", %{value: "c"})
