defmodule EventStore.IntegrationTest do
  ...
  describe "RMX005_5A_T10: Multiple event types" do
    test "different event types can coexist" do
      ...
      {:ok, _id, store} =
        EventStore.append(store, "base_card:1", "basecard.created", %{front: "A"})

      {:ok, _id, store} =
        EventStore.append(store, "base_card:1", "basecard.updated", %{back: "B"})
  ...
  describe "RMX005_5A_T11: Event type pattern querying" do
    test "can filter events by type pattern" do
      ...
      {:ok, _id, store} =
        EventStore.append(store, "base_card:1", "basecard.created", %{})

      {:ok, _id, store} =
        EventStore.append(store, "base_card:1", "basecard.updated", %{})

      {:ok, _id, store} = EventStore.append(store, "base_card:1", "basecard.deleted", %{})
  ...
  describe "RMX005_5A_T12: Timestamp ordering across events" do
    test "events have correct timestamp ordering" do
      ...
      {:ok, event_id_1, store} =
        EventStore.append(store, "entity:1", "event.type", %{order: 1})

      Process.sleep(10)

      {:ok, event_id_2, store} =
        EventStore.append(store, "entity:1", "event.type", %{order: 2})

      Process.sleep(10)

      {:ok, event_id_3, store} =
        EventStore.append(store, "entity:1", "event.type", %{order: 3})
