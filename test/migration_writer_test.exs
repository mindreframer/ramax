defmodule PState.MigrationWriterTest do
  ...
  test "RMX004_1A_T3: auto-flush triggers when batch_size reached" do
    ...
    assert {:ok, %{data: 1}} = pstate.adapter.get(pstate.adapter_state, pstate.space_id, "key1")
    assert {:ok, %{data: 2}} = pstate.adapter.get(pstate.adapter_state, pstate.space_id, "key2")
    assert {:ok, %{data: 3}} = pstate.adapter.get(pstate.adapter_state, pstate.space_id, "key3")
  end
  ...
  test "RMX004_1A_T4: timer-based flush works" do
    ...
    assert {:ok, %{data: "timer_test"}} =
             pstate.adapter.get(pstate.adapter_state, pstate.space_id, "timer_key")
  end
  ...
  test "RMX004_1A_T5: manual flush works", %{pstate: pstate} do
    ...
    # Verify data was written
    assert {:ok, %{id: "m1"}} = pstate.adapter.get(pstate.adapter_state, pstate.space_id, "manual1")
    assert {:ok, %{id: "m2"}} = pstate.adapter.get(pstate.adapter_state, pstate.space_id, "manual2")
  end
  ...
  test "RMX004_1A_T7: integration with fetch_and_auto_migrate" do
    ...
    {:ok, raw_data} = pstate.adapter.get(pstate.adapter_state, pstate.space_id, "base_card:card1")
