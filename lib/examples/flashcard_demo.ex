defmodule FlashcardDemo do
  ...
  def getapp do
    ...
    existing_events = File.exists?(event_db_path)
    existing_pstate = File.exists?(pstate_db_path)

    if existing_events or existing_pstate do
      IO.puts("📦 Loading existing SQLite databases...")
      IO.puts("  Events: #{event_db_path} #{if existing_events, do: "(exists)", else: "(new)"}")
      IO.puts("  PState: #{pstate_db_path} #{if existing_pstate, do: "(exists)", else: "(new)"}")
    else
      IO.puts("📦 Initializing new SQLite databases...")
  ...
  def run do
    ...
    # Initialize app with SQLite adapters (fixed paths for persistence)
    event_db_path = "/tmp/flashcard_demo_events.db"
    pstate_db_path = "/tmp/flashcard_demo_pstate.db"
    ...
    if event_count > 0 do
      IO.puts("⚠️  Database already contains #{event_count} events!")
      IO.puts("⚠️  Demo will add MORE data to existing database.")
      IO.puts("⚠️  To start fresh: rm #{event_db_path} #{pstate_db_path}\n")
    end
    ...
    {time_deck, {:ok, app}} =
      :timer.tc(fn -> FlashcardApp.create_deck(app, "spanish-101", "Spanish Basics") end)
    ...
    IO.puts(
      "  Total time for #{num_translations} English translations: #{time_en_trans / 1000}ms\n"
    )
    ...
    IO.puts(
      "  Total time for #{num_fr_translations} French translations: #{time_fr_trans / 1000}ms\n"
    )
    ...
    {time_update, {:ok, app}} =
      :timer.tc(fn -> FlashcardApp.update_card(app, "card-1", "Hello!", "¡Hola!") end)
    ...
    {time_list, {:ok, cards}} =
      :timer.tc(fn -> FlashcardApp.list_deck_cards(app, "spanish-101") end)
    ...
    {time_verify, {deck_after_rebuild, cards_after_rebuild}} =
      :timer.tc(fn ->
        {:ok, deck} = FlashcardApp.get_deck(app, "spanish-101")
        {:ok, cards} = FlashcardApp.list_deck_cards(app, "spanish-101")
        {deck, cards}
    ...
    IO.puts(
      "  Create cards (#{num_cards}): #{time_cards / 1000}ms (#{Float.round(num_cards / (time_cards / 1_000_000), 1)} cards/sec)"
    )
    ...
    total_time =
      time_init + time_deck + time_cards + time_en_trans + time_fr_trans +
